...
from config import OUTPUT_DIR, SCHEMA_DIR, FORKS
...
def read_backup(input_file: str) -> str | bytes:
  out_file = OUTPUT_DIR / 'extracted_tachibk'
...
def parse_backup(backup_data: str | bytes, fork: str = 'mihon') -> Backup:
...
def parse_json(input_file: str, fork: str = 'mihon') -> bytes:
  try:
...
def main() -> None:
  ...
  if input_file.endswith('.json'):
    output = OUTPUT_DIR / 'output.tachibk' if args.output == '.' else Path(args.output)
    write_backup(parse_json(input_file, args.fork), output)
  else:
    output = OUTPUT_DIR / 'output.json' if args.output == '.' else Path(args.output)
    write_json(parse_backup(read_backup(input_file), args.fork), output)
